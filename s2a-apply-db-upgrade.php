 ...
 * @version        SVN: $Revision: 550 $
 * @last-modified  SVN: $Date: 2007-02-28 15:15:08 -0800 (Wed, 28 Feb 2007) $
 */
...
if(count($module_changes['master_modules']) > 0){
    if(prompt("Import new master data?")){
         ...
         */
        if(false !== (strpos(strtolower($_ENV['OS']), 'win'))){

            $mysql_start_folder = 'C:\\Program Files\\MySQL';

            if(!$mysql_folder = findMySQLexe($mysql_start_folder)){
        ...
        /*$mastertemp_structure = file_get_contents($mastertemp_structure_file);
        $mastertemp_structure_sqls = explode("-- statement separator --", $mastertemp_structure);
        foreach($mastertemp_structure_sqls as $mastertemp_structure_sql){
            $mastertemp_structure_sql = trim($mastertemp_structure_sql);
            if(!empty($mastertemp_structure_sql)){
                print "executing $mastertemp_structure_sql\n";
                $r = $dbh->query($mastertemp_structure_sql);
                dbErrorCheck($r);
            }
        }
        $mastertemp_data = file_get_contents($mastertemp_data_file);
        $mastertemp_data_sqls = explode("-- statement separator --", $mastertemp_data);
        unset($mastertemp_data_sqls[0]); //removes the preable stuff.
        foreach($mastertemp_data_sqls as $mastertemp_data_sql){
            $mastertemp_data_sql = trim($mastertemp_data_sql);
            if(!empty($mastertemp_data_sql)){
                print "executing $mastertemp_data_sql\n";
                $r = $dbh->query($mastertemp_data_sql);
                dbErrorCheck($r);
            }
        }*/
        ...
        foreach($module_changes['master_modules'] as $master_module_id){
            ...
            /*$r = $dbh->query("LOCK TABLES `$master_module_id` WRITE");
            dbErrorCheck($r);*/
