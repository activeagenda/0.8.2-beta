...
function Statement($element, $moduleID)
{
    foreach($element->c as $subElement){
        ...
        case 'Field':
        case 'Value':
            $this->fields[$subElement->name] = $subElement->createObject($this->moduleID, null, &$this);
            break;
        case 'ModuleRef':
            if(empty($this->_rootModuleID)){
                $this->_rootModuleID = $subElement->attributes['moduleID'];
            }
            $this->moduleRefs[$subElement->attributes['alias']] = $subElement->createObject($this->moduleID, null, &$this);
            break;
        case 'FilterCondition':
            $this->filters[] = $subElement->createObject($this->moduleID, null, &$this);
            break;
        default:
            break;
        }
...
function Union($element, $moduleID)
{
    ...
    foreach($firstStatement->fields as $stmtField){
        $unionField = new UnionField($stmtField->name, $this->alias, $stmtField->dataType);
