...
//print "creating $type {$this->name}\n";
        ...
        if($object !== false){
            ...
            die("Element_createObject: Factory method is not defined for the class $type.");
        }
    } else {
        die("Element_createObject: $type is not defined as a class");
    }   
}
...
//print "creating $type {$this->name}\n";
        ...
        if($object !== false){
            ...
            die("Element_createDoc: DocFactory method is not defined for the class $type.");
        }
    } else {
 ...
 */
function &selectElements($elementType, $attribute = null, $attributeValue = null, $returnOnFirst = false)
{
//  print "selectElements $element\n";
    $matches = array();

    foreach($this->c as $name => $obj){
        if(is_object($obj) && is_a($obj, 'element')){
            //print "looking for $element: {$obj->name} {$obj->type}\n";
            ...
            //keep looking
            if(count($obj->c) > 0){
                $matches = array_merge($matches, $obj->selectElements($elementType, $attribute, $attributeValue));
...
function selectFirstElement($element, $attribute = NULL, $attributeValue = NULL)
{
    
    $elements = $this->selectElements($element, $attribute, $attributeValue, true);
...
function &selectChildrenOfFirst($element, $attribute = NULL, $attributeValue = NULL)
{
    $element = $this->selectFirstElement($element, $attribute, $attributeValue);
    return $element->c;
}
} //end class Element
...
function parseXML($XMLData){
        ...
        xml_parse($parser, $XMLData, true) or die("Can't parse XML data.");
...
function parseXMLFile($fileName = null)
{
    ...
    if (file_exists($fileName)){
        $filedata = file_get_contents($fileName) or die("$fileName: Can't read XML data from file.");
...
/**
 * retained for legacy support until all references are removed
 */
function parseMe()
{
    $this->parseXMLFile();
...
function ModuleMap($moduleID)
{
    ...
    $this->XMLFileName = XML_PATH . "/{$this->moduleID}ModuleDef.xml";
}
...
function recurseMap($object, $level = 0)
{
    ...
    //$cvars = get_class_vars($class);
    $vars = get_object_vars($object);
    //$vars = array_diff($vars, $cvars);
