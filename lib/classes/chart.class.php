...
class Chart {
    ...
    function Factory($element, $moduleID)
    {
        ...
        if(count($element->c) > 0){
            foreach($element->c as $sub_element){
                ...
                case 'GroupByField':
                    $chart->groupByFields[$sub_element->name] = $sub_element->attributes['dateInterval'];
                    $chart->labelField = $sub_element->name;
                    $chart->dateInterval = $sub_element->attributes['dateInterval'];
                    break;
                case 'SummaryField':
                    $chart->summaryFields[$sub_element->name] = $sub_element->attributes['type'];
        ...
        if(!empty($element->attributes['subModuleID'])){
            $subModuleID = $element->attributes['subModuleID'];
            ...
            if('ParetoChart' == $element->type || 'pareto' == $element->attributes['mode'] ){
    ...
    function getInterval($max){
        ...
        //die("max=$max; exp=$exp; interval=$interval");
        return $interval;
    }
    ...
    function showError($errorType, $errorInfo = '')
    {
        ...
        $this->_Graph->addNew('title', array($msg, 11));
        $this->_Graph->done();
    }
...
class PieChart extends Chart {
    ...
    function PieChart($moduleID, $name, $title, $subModuleID, $mode)
    {
...
//exit;
        ...
        if(count($result) < $this->getMinimumDataPoints()){
...
die('test2');*/
        ...
        if($showTitle){
            $this->_Graph->addNew('title', array($this->title, 11));
        }
...
//print debug_r($result);
        ...
        if(!empty($this->dateInterval)){
            ...
            //print "from $fromLabel to $toLabel; dateInterval {$this->dateInterval}<br />\n";
            ...
            /*print debug_r($startParts);
            print debug_r($endParts);*/
            $labelParts = count($startParts);
            $labels = array();
            
            if(!empty($startParts)){
                if(intval($startParts[0]) == 0){
                    die('wrong start date (level 1)');
                }
                if(intval($endParts[0]) == 0){
                    die('wrong end date (level 1)');
                }
                for($label1 = intval($startParts[0]); $label1<=intval($endParts[0]);$label1++){
                    if($labelParts > 1){
                        ...
                        if(intval($startParts[1]) == 0){
                            die('wrong start date (level 2)');
                        }
                        if(intval($endParts[1]) == 0){
                            die('wrong end date (level 2)');
                        }
                        if($onFirst1){
                        ...
                        for($label2 = $start2; $label2<=$end2;$label2++){
                            ...
                            if($labelParts > 2){
                                ...
                                if(intval($startParts[2]) == 0){
                                    die('wrong start date (level 3)');
                                }
                                if(intval($endParts[2]) == 0){
                                    die('wrong end date (level 3)');
                                }
                                if($onFirst2){
        ...
        /*print debug_r($result, 'result');
        print debug_r($Dataset, 'Dataset');
        die();*/
        $max = $Dataset->maximumY();
        $interval = $this->getInterval($max);
        $AxisX =& $Plotarea->getAxis(IMAGE_GRAPH_AXIS_X);
