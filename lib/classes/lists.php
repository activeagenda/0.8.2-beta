 ...
 * @version        SVN: $Revision: 548 $
 * @last-modified  SVN: $Date: 2007-02-28 10:54:02 -0800 (Wed, 28 Feb 2007) $
 */
...
function getCountSQL()
{
    ...
    if(empty($this->countSQL)){
        ...
        return $this->countSQL . $listFilterSQL;
    }
    return $SQL;
} //end getCountSQL
...
function render($startRow = null, $defaultOrderBys = null)
{
    ...
    if($nRows > 0){
        ...
        if($this->gridType == 'edit' || $this->gridType == 'edit_nfe'){
            $content = sprintf(
                VIEWGRID_TABLE,
    ...
    foreach($fieldNames as $fieldName){
        ...
        if('hide' != $alignment){
            ...
            if(isset($orderBys[$fieldName])){
                ...
                if(!$desc){
                    $obName = '-'.$fieldName;
                    $obImg = "&nbsp;<img src=\"$theme_web/img/order_asc.gif\" height=\"16\" width=\"16\" />";
                } else {
                    $obName = $fieldName;
                    $obImg = "&nbsp;<img src=\"$theme_web/img/order_desc.gif\" height=\"16\" width=\"16\" />";
                }
            } else {
                $obName = $fieldName;
                $obImg = '';
            ...
            $content .= sprintf(
                ...
                $this->headerPhrases[$fieldName].$obImg
            ...
            if(isset($this->headerPhrases[$fieldName])){
                ...
                if('hide' != $alignment){
                    if($this->useBestPractices && 'IsBestPractice' == $fieldName){
                        if(1 == $fieldValue){
                            $fieldContent = '<img src="'.$theme_web.'/img/best_practice.png" alt="'.gettext("A best practice!").'"/>';
                        } else {
                            $fieldContent = '';
                        }
                    } else {
                        $fieldContent = fldFormat($this->listData->fieldTypes[$fieldName], $fieldValue);
