...
$project = $_SERVER[argv][2];  //folder location
if(empty($project)){
...
if(!prompt("\nIn order to install the Active Agenda database, the MySQL root password is required. Continue?")){
    die("$debug_prefix Database install was cancelled");
}
...
print "Now, we will set up the adminstrator user. This should be a real person (it is possible to set up more administrators in the aplpication, so it is not necessary to share passwords).\n\n";
...
$organization_id = $dh->saveRow(array('Name' => $organization_name), 0);
