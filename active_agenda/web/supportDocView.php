...
if($allowEdit){
    $editLink = "<a href=\"frames_popup.php?dest=edit&mdl=spt&rid={$r[0]['SupportDocumentID']}\" target=\"_blank\">".gettext("Edit documentation for this module")."</a><br />\n";
} else {
    $editLink = '';
}
...
foreach($r as $row){
    ...
    if('all' == $mode || $row['SectionID'] == $selectedSectionName || $selectedSectionID == $counter){
        ...
        if($allowEditSection && (!empty($row['SupportDocumentSectionID']))){
            $content .= "<a href=\"frames_popup.php?dest=edit&mdl=spts&rid={$row['SupportDocumentSectionID']}\" target=\"_blank\">".gettext("Edit this section")."</a><br />\n";
        }
