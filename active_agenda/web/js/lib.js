...
function editRow(rowID){
    ...
    d.innerHTML = '<br/>Getting data from server...<br/>';
...
function updateList(listDiv, callUrl){
    ...
    d = document.getElementById(listDiv);
    d.style.cursor = 'wait';
...
{
    if(all){
        alert('Dismissing the intro for all modules.');
        noIntro = 'allModules';
    } else {
        alert('Dismissing the intro for this module.');
        noIntro = moduleID;
    }
...
function fixListWrap(obj){
    ...
    if(offsetY > obj.offsetTop){
        obj.style.marginRight = '180px'; //ie6
        //obj.style.marginRight = '160px'; //test ie7
        return (document.body.clientWidth - 230) + 'px';
        //return (document.all.content.width - 180) + 'px';
    } else {
...
function layoutFix(){
    ...
    if(overlap > 0){
        oContent.style.width = oContent.offsetWidth + overlap;
    }
