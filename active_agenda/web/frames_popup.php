...
//special case for Code items
if('cod' == $ModuleID){
    $dest = "edit.php?mdl=codt&scr=Items&rid=".intval($_GET['cti']);
}
...
if (empty($_SESSION['User']) || intval($_SESSION['Timeout']) < time()) {
    die("Your session has timed out (or you have logged out). Please close this window and log in from the main window.");
}
